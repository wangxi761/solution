...
public class MergeSort implements Sort {
	...
	public void iteratorSort(int[] arr) {
		...
		for (int i = 2; i < arr.length * 2; i *= 2) {
			for (int j = 0; j < (arr.length + i - 1) / i; j++) {
				int start = i * j;
				int mid = start + i / 2 >= arr.length
						? arr.length - 1
						: start + i / 2;
				int end = i * (j + 1) - 1 >= arr.length
						? arr.length - 1
						: i * (j + 1) - 1;
	...
	public void recursiveSort(int[] arr) {
		int[] result = new int[arr.length];
		recursive(arr, result, 0, arr.length - 1);
	}
	...
	public void recursive(int[] arr, int[] result, int start, int end) {
		...
		recursive(arr, result, l1, r1);
		recursive(arr, result, l2, r2);
		int count = l1;
		while (l1 <= r1 && l2 <= r2) {
